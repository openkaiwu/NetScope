...
## V1.0 30-day history, replayable attribution chains and weekly/monthly reports (local development build)

- **30-day tiered history**: the last 24 hours keep 5-second raw samples, day 1–7 collapse to 30-second granularity and anything older collapses to 5-minute granularity (the 30-day tier). Compaction writes the bucket **average** back instead of keeping one sampled row, and is idempotent — running it again never changes values. Existing databases enter the new tiers automatically.
- **Replayable attribution chain**: event details expand into five steps — event → candidate processes → evidence → root-cause hypothesis → confidence. The chain derives only from the stored event, so reopening the same event always yields the same conclusion; when raw samples are past retention the chain says so instead of pretending. Event details now also show the 30-seconds-before/after system curves, and “Replay raw evidence” fetches the primary process's CPU, private-memory and I/O curves for the event window.
- **Recurrence statistics**: event details answer “does this happen often” — same-type events for the primary process over the past 30 days and how many of them overlapped a user lag mark.
...
- **Termination advice**: the process center shows a five-level recommendation (blocked / strongly discouraged / caution: possible data or function loss / usually terminable / unknown) with evidence (critical flag, owner, session, service hosting, signature, main window, ports and connections), unknowns and alternative actions. Termination policies for all 36 built-in system processes participate in grading; a system-name impostor (path/signature not matching system traits) degrades to “unknown”.
- **Request close / force terminate**: always triggered explicitly by the user in the foreground — never automatic. Every row in the high-impact and process-center lists also carries a quick force-terminate button: the target is assessed on click, ineligible processes are not executed and the app jumps to the advice card explaining why, while eligible ones go through the same confirmation and execution chain. Request close sends a normal close to the main window and waits for exit (apps may save or refuse); force terminate is only offered at caution/usually-terminable levels, and the caution level requires ticking “I have saved my work”. At execution time the target identity is re-verified — PID, creation time, critical flag, session/owner/service hosting — and a reused PID is refused outright. No elevation, no process trees, no services.
